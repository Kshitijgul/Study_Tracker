...
export default function App() {
  ...
  const handleImportData = useCallback((rawData) => {
    const importedDate = importData(rawData);
    syncMonthYearFromDate(importedDate);
  }, [importData, syncMonthYearFromDate]);

  const handleResetApp = useCallback(() => {
    const confirmed = window.confirm('This will permanently clear all subjects and tasks. Continue?');
    if (!confirmed) return;
  ...
  // Week stats
  const weekStats = useMemo(() => getWeekStats(weekDates), [getWeekStats, weekDates]);

  // Month stats
  const monthStats = useMemo(() => getMonthStats(currentYear, currentMonth), [getMonthStats, currentYear, currentMonth]);

  // Subject stats
  const subjectStats = useMemo(() => getSubjectStats(currentYear, currentMonth), [getSubjectStats, currentYear, currentMonth]);
        ...
        <div className="max-w-[1400px] mx-auto p-4 lg:p-6">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div className="lg:ml-0 ml-12">
              ...
              <p className="text-sm text-gray-500">Track your daily study progress — one task at a time</p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  const t = new Date();
                  const todayStr = formatDate(t.getFullYear(), t.getMonth(), t.getDate());
